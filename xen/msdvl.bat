...
for %%c in (%config_os%) do (
    ...
    for %%d in (%dvl_drv%) do (
        cd %%d
        if not %%d==xenbus (
            if not exist xenbus\%%c\%cp_platform% mkdir xenbus\%%c\%cp_platform%
            copy ..\xenbus\%%c\%cp_platform%\xenbus.lib xenbus\%%c\%cp_platform%
        )
        ...
        c:\codeql-home\codeql\codeql.cmd database create -l=cpp -s=C:\vmdp\xen\%%d -c "msbuild /t:rebuild "C:\vmdp\xen\%%d\%%d.vcxproj" /p:Configuration=%%c /p:Platform=x64 /p:UseSharedCompilation=false" "C:\codeql-home\databases\%%d" -j 0
        ...
        del %%d.dvl.xml
        if not %%d==xenbus del xenbus\%config_os%\%cp_platform%\xenbus.lib
        cd ..
    )
)
